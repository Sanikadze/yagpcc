...
func TestStatementsMapping_Golden(t *testing.T) {
	...
	// The extension may be older than schema v2: no planJson/analyzeJson keys
	// means NULL columns, and nothing else in the row shifts.
...
// planJSONSample is a short but structurally real EXPLAIN (FORMAT JSON) plan:
// the top-level array, a Plan node and a Gather Motion child — the node type
// that only survives the JSON path (the text plan drops it).
...
// planJSONDoc renders a source document with the given JSON payloads embedded
// as proper JSON string values, the way protojson emits them.
...
func TestStatementsMapping_PlanJSONColumns(t *testing.T) {
	...
	// Everything in the document is consumed by a column, so nothing is left
	// over: the FORMAT JSON payloads must not leak into _rest.
	assert.Nil(t, row[colRest])
...
func TestMapping_PlanJSONSnakeCaseAlias(t *testing.T) {
	// The session-style snake_case spelling (encoding/json tags) must match the
	// same columns as the protojson camelCase one.
...
func TestMapping_PlanJSONColumnPosition(t *testing.T) {
	// Insert order must follow the post-0002 DDL: plan_text, plan_json,
	// analyze_json, template_query_text.
	for _, tm := range []*tableMapping{StatementsMapping(), SegmentsMapping()} {
		names := tm.ColumnNames()
		idx := map[string]int{}
		for i, n := range names {
